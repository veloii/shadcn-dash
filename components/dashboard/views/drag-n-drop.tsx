  ...
  return (
    <div
      className={cn(
        ...
        !sameId &&
        active &&
        "before:h-1 before:rounded-full before:absolute before:bg-muted before:left-0 before:w-full before:[content:'']",
        ...
        <SortableContext items={store.views} strategy={rectSwappingStrategy}>
          <Comp
            className={cn(
              ...
              "rounded-lg border-dashed border-2 min-h-full transition-[border-color,background]",
              className,
            )}
            {...props}
            ref={setNodeRef}
...
export function Dnd({ children }: { children: React.ReactNode }) {
  ...
  const handleDragEnd = React.useCallback((event: DragEndEvent) => {
    ...
    const movedViewIndex = fromStore.views.findIndex(v => v.id === movedView.id)
    ...
    const fromStoreNewView = fromStore.views[movedViewIndex + 1] || fromStore.views[movedViewIndex - 1];
