...
import { ViewStoreProvider, useViewStore } from '@/components/dashboard/views/store'
...
function ViewsContainer({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className={cn(type === "top" ? "flex flex-col divide-y" : "divide-x flex flex-row", "border rounded-lg")}>
      {children}
    </div>
  )
}
